...
# df_mort <- read_rds(file.path(processed_data_path, "Mortality_2015_GH.rds"))
# ## read in covariate data
# df_covar <- read_rds(file.path(processed_data_path, "Covariate_GH.rds"))
# 
...
for(k in 1:nfolds){
        ...
        # save concordance for cox model
        if(!inherits(fit_k,"try-error")){
                ...
                # ID_AUC(df_emp$eta, df_emp$time, df_emp$event, ut_train[2])
