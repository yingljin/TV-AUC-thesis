...
Since we have generated to data from proportional hazard model with a defined baseline hazard, we can write the conditional density of event time as $f(t| \eta) = \lambda(t|\eta)S(t|\eta) = \lambda_0(t)S_0(t)exp(2\eta)$. Subsequently, we can derive the formulas for sensitivity (TP) and specificity (1-FP) based on Bayes' rule:
...
Therefore, we could plot out the ROC curve at a specific time point, and get $\text{AUC}^{\mathbb{I}/\mathbb{D}}(t)$ through numeric integral over all possible thresholds of biomarker: 
