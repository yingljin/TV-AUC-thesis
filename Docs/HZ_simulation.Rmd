...
#load(here("outputData/estimates_N_500.RData"))
load(here("outputData/estimates_N_500.RData"))
head(table(auc_df$iter))
```
...
```{r tvauc_boxplots, fig.height=4, fig.width=12}
brk <- seq(0, 1, 0.2)
tvauc_box1 <- auc_df_long %>%
  dplyr::select(-true) %>%
...
ggarrange(tvauc_box1, tvauc_box2, nrow = 1, common.legend = T)
```
...
ggarrange(c1, c2, nrow = 1, common.legend = T)
```
