...
tv_auc_plot <- ggarrange(p1, p2, p3, nrow = 3, ncol = 1, common.legend = T)
annotate_figure(tv_auc_plot, bottom = "Note: black line indicates the true AUC value")
```


```{r compiled_tv_auc}
p_hz <- auc_df %>% dplyr::select(time, contains("HZ"), true, sample) %>%
  pivot_longer(2:4, names_to = "model", values_to = "auc") %>%
  mutate(model = factor(model, levels = c("HZ_eta1", "HZ_eta2", "HZ_eta3"),
                            labels = c("No noise", "20 noise signals", "100 noise signals"))) %>%
  group_by(sample, model) %>%
  ggplot()+
  geom_smooth(aes(x = time, y = auc, col = model, linetype = sample), 
              se = F, formula = y~s(x, k=30, bs = "cs"), na.rm = T, method = "gam")+
  geom_line(aes(x = time, y = true), na.rm = T)+
  labs(title = "Semi-parametric estimators", x = "Time", y = "AUC")+
...
```{r}
p_np <- auc_df %>% dplyr::select(time, starts_with("empirical_"), true, sample) %>%
  pivot_longer(2:4, names_to = "model", values_to = "auc") %>%
  mutate(model = factor(model, 
                        levels = c("empirical_eta1", 
                                   "empirical_eta2", 
                                   "empirical_eta3"),
                            labels = c("No noise", 
                                       "20 noise signals", 
  ...
  geom_smooth(aes(x = time, y = auc, col = model, linetype = sample), 
              se = F, formula = y~s(x, k=30, bs = "cs"), na.rm = T, method = "gam")+
  geom_line(aes(x = time, y = true), na.rm = T)+
  labs(title = "Non-parametric estimators", x = "Time", y = "AUC")+
  coord_cartesian(ylim=c(0.6, 1))

#range(auc_df$empirical_eta1[auc_df$sample == "in-sample"])

```


```{r}
p_snp <- auc_df %>% dplyr::select(time, starts_with("sm_empirical_"), true, sample) %>%
  pivot_longer(2:4, names_to = "model", values_to = "auc") %>%
  mutate(model = factor(model, levels = c("sm_empirical_eta1", "sm_empirical_eta2", "sm_empirical_eta3"),
                            labels = c("No noise", "20 noise signals", "100 noise signals"))) %>%
  group_by(sample, model) %>%
  ggplot()+
  geom_smooth(aes(x = time, y = auc, col = model, linetype = sample), 
              se = F, formula = y~s(x, k=30, bs = "cs"), na.rm = T, method = "gam")+
  geom_line(aes(x = time, y = true), na.rm = T)+
  labs(title = "Smoothed non-parametric estimators", x = "Time", y = "AUC")+
  coord_cartesian(ylim=c(0.6, 1))
```

```{r}

tv_auc_plot_compile <- ggarrange(p_hz,  p_np, p_snp, nrow = 1, ncol = 3, common.legend = T,
                                 legend = "right")
```

```{r fig.width=6, fig.height=2}
annotate_figure(tv_auc_plot_compile, bottom = "Note: black line indicates the true AUC value")
```
...
bp_in <- auc_df %>%
  ...
  pivot_longer(3:11, names_to = "estimator", values_to = "auc") %>%
  mutate(model = gsub("HZ_|empirical_|sm_empirical_", "", estimator)) %>%
  mutate(estimator = gsub("_eta1|_eta2|_eta3", "", estimator)) %>%
  mutate(model = factor(model, levels = c("eta1", "eta2", "eta3"),
                        labels = c("No noise", "20 noise signals", "100 noise signals"))) %>%
  mutate(estimator = factor(estimator, levels = c("HZ", "empirical", "sm_empirical"),
                            labels = c("Semi-parametric", "Non-parametric", "Smoothed non-parametric"))) %>%
  ggplot(aes(x = factor(time_bin), y = auc, fill = estimator))+
  geom_boxplot(outlier.size = 0.5)+
  facet_wrap(~model, nrow = 1, ncol = 3)+
  labs(x = "Time", y = "AUC", title = "In-sample estimates")+
...
bp_out<- auc_df %>%
  ...
  mutate(estimator = factor(estimator, 
                            levels = c("HZ", "empirical", "sm_empirical"),
                            labels = c("Semi-parametric", "Non-parametric", "Smoothed non-parametric"))) %>%
  ggplot(aes(x = factor(time_bin), y = auc, fill = estimator))+
  geom_boxplot(outlier.size = 0.5)+
  facet_wrap(~model, nrow = 1, ncol = 3)+
  labs(x = "Time", y = "AUC", title = "Out-of-sample estimates")+
  theme(axis.text.x = element_text(angle = 60, vjust = 0.1, hjust = 0.1))
...
tv_auc_boxplot_compile <- ggarrange(bp_in,  bp_out, nrow = 1, ncol = 2, common.legend = T)
```

```{r, fig.width=7, fig.height=3}
tv_auc_boxplot_compile
```
...
c_plot <- ggarrange(pc1, pc2, nrow = 1, common.legend = T)
annotate_figure(c_plot, bottom = "Note: red line indicates the true concordance")
```
