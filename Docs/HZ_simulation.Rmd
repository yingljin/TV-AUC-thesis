...
bp_in <- auc_df %>%
  ...
  mutate(model = factor(model, levels = c("eta1", "eta2", "eta3"),
                        labels = c("No noise", "20 noise signals", "100 noise signals"))) %>%
  mutate(estimator = factor(estimator, levels = c("HZ", "sm_empirical", "empirical"),
                            labels = c("Semi-parametric", "Smoothed non-parametric", "Non-parametric"))) %>%
...
bp_out<- auc_df %>%
  ...
  mutate(estimator = factor(estimator, 
                            levels = c("HZ", "sm_empirical", "empirical"),
                            labels = c("Semi-parametric", "Smoothed non-parametric", "Non-parametric"))) %>%
  ggplot(aes(x = factor(time_bin), y = auc, fill = estimator))+
  geom_boxplot(outlier.size = 0.5)+
  facet_wrap(~model, nrow = 1, ncol = 3)+
