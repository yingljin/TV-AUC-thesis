...
We are trying to identifying key flaw in HZ estimators of I/D AUC and their impact on Concordance, also compare prediction performance of common estimators with the presence of noise.
...
```{r}
auc_df %>% dplyr::select(time, contains("_eta1"), true, sample) %>%
...
```{r}
auc_df %>% dplyr::select(time, contains("_eta2"), true, sample) %>%
  pivot_longer(2:4, names_to = "estimator", values_to = "auc") %>%
...
```{r}
auc_df %>% dplyr::select(time, contains("_eta3"), true, sample) %>%
  pivot_longer(2:4, names_to = "estimator", values_to = "auc") %>%
