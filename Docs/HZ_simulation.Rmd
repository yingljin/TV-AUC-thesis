...
For each of the 1000 simulated data sets with sample size N = 500, we generated three covariates $\boldsymbol{X} = (X_1, X_2, X_3)$ independently from standard normal distribution and set the true value of coefficients to be (1, -1, 0.25). Therefore, the risk score $\eta = \boldsymbol{X^t\beta}$ follows a normal distribution with mean = 0 and variance of 2.0625. The survival time $T$ is generated from a propotional hazard model with Weibull baseline hazard $\lambda_0(t) = p\lambda t^{p-1}$, thus the baseline survival function $S_0(t) = exp(-\lambda p^t)$, and survival function conditioning on covariates is $logS(t|\boldsymbol{X}) = exp(\boldsymbol{X^t\beta})logS_0(t)$. In addition, the censor time is generated independently from survival time and uniformly between (0.5, 1). 

To investigate the effect of noise on estimators, we constructed three different models: 1) model correctly specified with the three covariates; 2) model with 20 additional noise, generated identically and independently with covariate; and 3) model with 100 additional noise signals. The additional noise signals are not associated with outcome, thus should not contributed to the discriminative performance of underlying models. Therefore the true AUC and concordance values should not change across three models. 
...
colnames(compare_auc) <- c("Estimator", "Bias", "Variability", "Inflation with noise") 
...
```{r}
compare_c <- data.frame(Estimator = c("Harrell's C", "Gonen-Heller", 
                                      "HZ_S","HZ_SmS",
                                      "NP_S","NP_SmS",
                                      "SNP_S","SNP_SmS"))

compare_c[, 'In-sample bias'] <- c("Overestimation", rep("No bias", 3), "Overestimation", rep("No bias", 3))
compare_c[, 'Out-of-sample bias'] <- c("Overestimation, but smaller than in-sample", rep("No bias", 3), "Overestimation, , but smaller than in-sample", rep("No bias", 3))
...
Below we present more detailed results from simulation. For the purpose of better presentation, incident/dynamic AUC estimates across all simulation is smoothed over time with generalized additive model in the following figure, since the event times generated in each simulation are different. As is revealed, both semi- and non-parametric estimators are unbiased and close to the true when the model is specified without noise, while smoothed non-parametric estimator seems to underestimate true discriminative performance at the beginning and end of follow-up period, but overestimate in the middle. However, as we include more noise signals in the model, both in-sample and out-of-sample semi-parametric estimates are significantly inflated to be much higher than the truth, indicating overestimation of the discriminative performance of underlying models.
...
p1 <- auc_df %>% dplyr::select(time, contains("_eta1"), true, sample) %>%
  ...
  labs(title = "No noise", x = "Time", y = "AUC")
```
...
p2 <- auc_df %>% dplyr::select(time, contains("_eta2"), true, sample) %>%
  ...
  geom_line(aes(x = time, y = true), na.rm = T)+
  facet_wrap(~estimator)+
  labs(title = "20 noise signals", x = "Time", y = "AUC")
```
...
p3 <- auc_df %>% dplyr::select(time, contains("_eta3"), true, sample) %>%
  ...
  geom_line(aes(x = time, y = true), na.rm = T)+
  facet_wrap(~estimator)+
  labs(title = "100 noise signals", x = "Time", y = "AUC")
```
...
From the figure below, most estimators of concordance are unbiased when the model is specified correctly. However, the Harrell' c-index and concordance from integrating non-parametric $\text{AUC}^{\mathbb{I}/\mathbb{D}}(t)$ estimator with weights from estimated Kaplan-Meier curve (NP_S) is biased upwards. The non-parametric estimators are also more variable, but smoothing the survival curve for integral can significantly reduce there variability (NP_SmS). It is also worth noting that the out-of-sample bias of Harrell's c and NP_S is smaller than in-sample bias. However, as more noise signals are included in the model, all the semi-parametric estimators (Gonen-Heller, HZ_S and HZ_SmS) are also significantly inflated above the truth, and the fully non-parametric estimators overfit to the training data. 
...
c_df_train$estimator <- factor(c_df_train$estimator, levels = levels(c_df_train$estimator),
                         labels = c("Harrell's C", "Gonen-Heller", "HZ_S",
                                    "HZ_SmS",
                                    "NP_S",
                                    "NP_SmS",
                                    "SNP_S",
...
c_df_train$estimator <- factor(c_df_train$estimator,
                               levels =  c("Gonen-Heller", "HZ_S", "HZ_SmS",
                                           "Harrell's C", "NP_S","NP_SmS","SNP_S", "SNP_SmS"))
# color by proporty
c_df_train$prop <- as.factor(ifelse(c_df_train$estimator %in% c("Gonen-Heller", "HZ_S", "HZ_SmS"), 
                          "Semi-parametric", "Non-parametric"))
text_col <- c(rep("#00BFC4", 3), rep("#F8766D", 5))
...
pc1 <- c_df_train %>%  ggplot(aes(x = estimator, y = concordance))+
  geom_boxplot(aes(fill = prop))+
  theme(axis.text.x = element_text(angle = 70, vjust = 0.5, color = text_col), 
        legend.title = element_blank())+
  geom_hline(yintercept = true_c, col = "red")+
  facet_wrap(~signal)+
  labs(title = "In-sample concordance", x = "", y = "Concordance")
...
c_df_test$estimator <- factor(c_df_test$estimator,
                               levels =  c("Gonen-Heller", "HZ_S", "HZ_SmS",
                                           "Harrell's C", "NP_S","NP_SmS","SNP_S", "SNP_SmS"))
c_df_test$prop <- as.factor(ifelse(c_df_test$estimator %in% c("Gonen-Heller", "HZ_S", "HZ_SmS"), 
                          "Semi-parametric", "Non-parametric"))

c_df_test$signal <- factor(c_df_test$signal, levels = levels(as.factor(c_df_test$signal)),
                           labels = c("No noise", "20 noise", "100 noise"))
pc2 <- c_df_test %>%  ggplot(aes(x = estimator, y = concordance))+
  geom_boxplot(aes(fill = prop))+
  theme(axis.text.x = element_text(angle = 70, vjust = 0.5, color = text_col),
        legend.title = element_blank())+
  geom_hline(yintercept = true_c, col = "red")+
  facet_wrap(~signal)+
  labs(title = "Out of sample concordance", x = "", y = "Concordance")
```

```{r, fig.width=10, fig.height=5}
ggpubr::ggarrange(pc1, pc2, nrow = 1, common.legend = T)
```

# Discussion

We would like to further investigate how variable selection methods (e.g. ridge regression) would affect estimators of $\text{AUC}^{\mathbb{I}/\mathbb{D}}(t)$ and concordance. Since out-of-sample inflation was caused by model overfit to the training data, we anticipate using LASSO or ridge regression in model fit would metigate this inflation. 

We would also attempt to mitigate bias of smoothed non-parametric estimator (SNP) of $\text{AUC}^{\mathbb{I}/\mathbb{D}}(t)$. One possible cause for bias is the skewed distribution of $\text{AUC}^{\mathbb{I}/\mathbb{D}}(t)$, which also seems to be bounded and centered around a specific value with small variance. We have found so far that neither transformation of outcome nor weighted regression by variance was helpful, while use a bounded regression  in smoothing (e.g. beta distribution) led to slight improvement. In addition, while non-parametric concordance seems to have upward bias, smoothness of survival function for weights of integral of $\text{AUC}^{\mathbb{I}/\mathbb{D}}(t)$ seems to reduce such bias. The reason for this phenomina is not clear to us.
