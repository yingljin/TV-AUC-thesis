...
```{r SimDataDensity, class.source='fold-show', fig.width=8, fig.height=3}
...
all_train_data %>%
  ...
  ggplot()+
  geom_histogram(aes(x=time, y=after_stat(ndensity)), bins=50)+
  facet_wrap(~event)+
...
```{r eval=FALSE}
# bias across all simulation, no noise model only
tv_auc_df %>% 
  # filter(model != "20 noise lasso" & model != "100 noise lasso") %>%
  mutate(model = factor(model, 
                        levels = c("No noise", "20 noise", "100 noise"))) %>%
  left_join(true_auc_df, by = "time") %>% 
  pivot_longer(3:5, names_to = "estimator", values_to = "AUC") %>% 
  mutate(bias = AUC-true_auc) %>% group_by(estimator) %>%
  filter(model == "No noise") %>% 
  ggplot(aes(x=estimator, y=bias, group=estimator))+
  geom_boxplot()+
   stat_summary(fun=mean, colour="red", geom="point", alpha = 0.5,
...
tv_auc_df2 %>% 
  ...
  facet_wrap(~Estimator, nrow = 1)+
  scale_color_manual(values = cbPalette)+
  labs(col = "Misalignment", linetype = "Sample", x = "Time")+
  theme(axis.text.x = element_text(vjust = 0.9))
```
...
appl_auc %>% 
  ggplot(aes(x = time, y = AUC, col = model, linetype = sample, linewidth = sample,
             alpha = sample))+
...
```{r}
# summarizing table of average out-of-sample concordance across filed
appl_c %>% filter(Sample == "Out-of-sample") %>% 
  group_by(estimator, model) %>% 
  summarize(mean_c = mean(Freq))

```

```{r auc_t_scatter, fig.width=10, fig.height=3}
appl_auc %>% filter(sample == "In-sample") %>%
  mutate(iter=factor(iter, levels = 1:10)) %>%
  ggplot(aes(x=time, y=AUC, color = iter))+
  geom_point(size = 0.5, alpha = 0.8)+
  facet_wrap(~Estimator, nrow = 1)+
  labs(x = "Time", y = "AUC", color = "Fold")  +
  scale_color_manual(values =  c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442",
                                 "#0072B2", "#D55E00", "#CC79A7", "#44AA99", "#88CCEE")
)
```
