...
tv_auc_df <- bind_rows(auc_lst) 
c_df <- bind_rows(c_lst)
```
...
tv_auc_df %>%
  ...
  mutate(Estimator = factor(Estimator, levels = c("HZ", "NP", "SNP"),
                             labels = c("Semi-parametric", 
                                       ...
                                       "Smoothed non-parametric"))) %>%
  ggplot()+
  geom_boxplot(aes(x=time_bin, y=AUC, col=Estimator, fill = Estimator),
...
c_df %>% 
  ...
  ggplot()+
  geom_boxplot(aes(x=outlier, y=Concordance, fill=Sample, col=Sample), na.rm = T)+
...
tv_auc_df %>% 
  ...
  mutate(Estimator = factor(Estimator, levels = c("HZ", "NP", "SNP"),
                             labels = c("Semi-parametric", 
                                       ...
                                       "Smoothed non-parametric"))) %>%
  mutate(time_bin = cut(time, breaks = seq(0, 1, 0.2), include.lowest = T)) %>%
  ggplot(aes(x = factor(time_bin), y = AUC, fill = Estimator, col=Estimator))+
...
c_df %>% 
  ...
  rename(Sample = sample) %>%
  ggplot()+
  geom_boxplot(aes(x=model, y=Concordance, fill=Sample, col=Sample), na.rm = T)+
  geom_hline(yintercept = true_c, col = "black")+
  facet_wrap(~type+estimator, nrow = 1)+
  labs(x = "Model")+
...
appl_c%>%
  ggplot() + 
  geom_boxplot(aes(x=model,y=Freq, fill=Sample, col= Sample)) + 
  facet_wrap(~type+estimator, nrow = 1)+
