...
In this section, we illustrate and compared the behavior of estimates under the effect of data contamination. At each iteration we use the same training set, and introduce outliers to the testing data set based on the following scenarios: 

- Mean shift: Outliers are generated from N(5, 1)
- Large variation: Outliers are generated from N(0, 5)

First, we create artifical outliers in the test data: 

```{r}
# test data 0: no outlier
data_train <- data_train %>% select(-Z) 
data_test  <- data_test %>% select(-Z)
...
```{r}
data.frame(none = data_test$true_eta, 
           mean_ch = data_test1$true_eta, 
           sd_ch = data_test2$true_eta) %>%
  pivot_longer(1:3) %>%
  mutate(name = factor(name, levels = c("none", "mean_ch", "sd_ch"),
                       labels = c("None", "N(5, 1)", "N(0, 5)"))) %>%
